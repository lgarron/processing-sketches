...
void center(float p) {
  // float sc = (1-constrain(t*2, 0, 1));
  triangle(0, 0, 0, s1(), -s1(), 0);

  float p2 = constrain(p*2, 0, 1);
  float diag = s1()*((1-p2) + 0.5*pow(0.5, 0.5) * p2);
  triangle(-diag, diag, 0, s1(), -s1(), 0);
}
